...
- Preserve the two-step public contract in every test and release: first
  ...
  asset id; Fabric returns only its `did:web`. The opaque hash stops at Fabric.
  Resolve that DID, use authenticated IHE PDQm `POST Patient/$match` with one
  FHIR `Parameters` body for the provider lookup, and only then ask an available
  trusted tenant to issue the token.
