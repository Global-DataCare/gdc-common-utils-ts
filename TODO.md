...
6. Expand the high-level bundle editing/building surface beyond the current employee-first implementation:
   - add typed entry editors/builders for `RelatedPerson`, `License`, `Offer`, and `Order`
   - keep `newEntry() -> asX() -> set/get -> doneEntry()` symmetry with `EmployeeEntryEditor`
   - add canonical bundle builders for license search and other non-employee controller flows
   - avoid forcing downstream packages to write raw `type`, `request.method`, and `meta.claims` shapes inline
7. Add canonical license query/summary primitives used by frontend, GW, and SDK layers:
   - search/filter DTOs for `status`, `subjectId`, `email`, `role`, `userClass`, and app `type`
   - summary DTOs for contracted / used / free seats
   - deterministic helpers for recognizing controller-auto-consumed first seats vs later invitations
   - response-normalization helpers from raw GW/device-license records to app-facing summaries
8. Add canonical consent-management primitives used by frontend, GW, and SDK layers:
...
9. Add canonical consent query/filter helpers for frontend-managed collections:
...
10. Add canonical clinical import builders for "Agregar datos" flows:
...
11. Add canonical `RelatedPerson` classification/filter helpers:
...
12. Add canonical unified-view / IPS resource helpers:
...
13. Start dataspace discovery foundation:
   - EU coverage helpers
...
14. Finish onboarding/discovery claim cleanup:
...
15. Keep repo scope boundaries explicit:
...
16. Keep `101-*` tests aligned with public editing surfaces:
