...
const bundleEditor = new CommunicationAttachedBundleSession({ communicationClaims });
...
`CommunicationAttachedBundleSession` is the technical API type, but in developer docs the better mental model is `bundleEditor`: an in-memory editor for the Bundle carried by a `Communication`.
