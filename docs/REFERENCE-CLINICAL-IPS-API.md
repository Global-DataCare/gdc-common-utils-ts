...
| `MedicationStatement.code` | `setCode(...)` | `getCode()` | `HealthcareBasicSections.HistoryOfMedicationUse.attributeValue` | `code` (token to `medication.concept`) | no | yes | use with `code-text` and `code-display` when coding exists |
| `MedicationStatement.code-text` | `setCodeTextLocal(...)` / legacy `setMedicationText(...)` | `getCodeTextLocal()` / `getMedicationText()` | `HealthcareBasicSections.HistoryOfMedicationUse.attributeValue` | companion, not a SearchParameter | no | yes | canonical local CodeableConcept.text; `medication-text` is read-only compatibility |
| `MedicationStatement.code-display` | `setCodeDisplay(...)` | `getCodeDisplay()` | `HealthcareBasicSections.HistoryOfMedicationUse.attributeValue` | companion, not a SearchParameter | no | yes | terminology Coding.display |
| `MedicationStatement.medication` | `setMedication(...)` | `getMedication()` | `HealthcareBasicSections.HistoryOfMedicationUse.attributeValue` | `medication` (reference) | no | yes | maps to `medication.reference`, never aliases `code` |
| `MedicationStatement.adherence` | `setAdherence(...)` | `getAdherence()` | `HealthcareBasicSections.HistoryOfMedicationUse.attributeValue` | `adherence` (R5 token) | no | yes | HL7 expression targets R5 `adherence.code`, but that dotted expression is never a claim key; use `adherence-text` / `adherence-display` companions |
